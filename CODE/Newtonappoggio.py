...
class Newton:
        ...
        def __init__(self,xtot,n,f0,f_1,g_1,x,x_1,eps,maxiter,iprint,funct,d1,d2,n_iter_glob,var):
            self.n = n
            self.xtot=xtot
            self.f0=f0
            self.f_1=f_1
        ...
        def gradiente_Hessiano_B(self,x, n ,f, f_1, g_1, funct, d1,d2, alpha, beta,n_iter_glob):
        ...
        def gradiente_Hessiano_M(self,x, n ,f, f_1, funct, d1,d2, alpha, beta,n_iter_glob):
...
#            if np.abs(beta) <= 1.e-16:
#                beta = min_sample_val if beta >= 0 else  -min_sample_val                                	
            fA = f
            fB = f_1
            fC = self.funct(x + alpha* d1 + beta * d2)
            if fC == np.inf or np.isnan(fC)  or abs(fC-f) > np.maximum(abs(f),1.)*1.e3:
                alpha=1.e-3*alpha
                beta=1.e-3*beta
                fC = self.funct(x + alpha* d1 + beta * d2)
            fD = self.funct(x + alpha* d1)                
            #print('fD=',fD) 
        ...
        def linesearch_armijo(self,xtot,d1,d2,n,x,f,d,gd,gamma,nf,funct):
            ...
            y=x+alfa*d
            f_alfa=funct(xtot + y[0]* d1 + y[1] * d2)
            f_min=f_alfa
            alfa_min=alfa
...
#            f=funct(x)
            ...
            while True:
                ...
                if (var == 1):  
                   g,H=self.gradiente_Hessiano_M( xtot, n, f, f_1, funct, d1,d2, x_1[0], x_1[1],n_iter_glob)
...
#                   input()
                ...
                nc=0
                if nc==0:    
                    alfa,f_alfa,nf=self.linesearch_armijo(xtot,d1,d2,n,x,f,d,gd,gamma,nf,funct)
