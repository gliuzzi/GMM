...
class Newton:
            ...
            if var < 7:     
                if n_iter_glob == 0:
                    ...
                    if var==3:
                        ...
                        #uv=np.where(muv<=0,muv,np.where(np.abs(muv)<eps,eps,muv))
                        ##muv=np.where(muv<=0,sv2,np.where(np.abs(muv)<eps,eps,muv))
                        #muv=np.where(muv>=0,muv,np.where(np.abs(muv)>1./eps,-1./eps,muv))
                        #muv=np.where(muv<=0,muv,np.where(np.abs(muv)>1./eps,1./eps,muv)) 
                        ##muv=np.where(muv<=0,sv2,np.where(np.abs(muv)>1./eps,1./eps,muv))    
                        ba=np.sum(muv*d1**2)
                        bc=np.sum(muv*d2**2)
                        bb=np.sum(muv*d1*d2)
                    if var==4:
        ...
        def gradiente_Hessiano_Hd(self, n, f, f_1, funct,x_glob, grad, d1,d2, alpha, beta,n_iter_glob,d1_norm,d2_norm):
            ...
            #gp = grad(x_glob+eta*(-d1))              
            #ba = np.dot((-d1).T,(g_p+d1)/eta)
            #if np.isnan(d2_norm) or d2_norm==0.:
                #bb = 0.
                #bc = 0.
            #else:
                #bb = np.dot((d2).T,(g_p+d1)/eta) 
                #gp = grad(x_glob+eta*(d2)) 
                #bc = np.dot((d2).T,(g_p+d1)/eta)
            #Bab = np.array([[ba,bb],[bb,bc]])            
...
#            f=funct(x)
            ...
            while True:
                ...
                if (var == 2):                
                    ...
                    if var >= 3:
                        d,gd,nc = self.direction(g,H,norma_g,n,x,n_iter_glob,var,d2,d1_norm)
                    else:
                        d,gd,nc = self.direction(g,H,norma_g,n,x,n_iter_glob,var,d2,d2_norm)
