...
from Newton_tot_7_ultimo import *
...
class Solver:
    ...
    def solve(self, method=None, eps_grad=None, max_iters=None, gamma=None, min_step=None, gtol_ord=None, recovery_steps=None):
        ...
        elif  self.method == 'QPS-randdir':
            sol, info = self.solvePlaneSearch_randdir()
        ...
        elif  self.method == 'QPS-Diagonale1':
            sol, info = self.solvePlaneSearch_Newton(var=3)
        elif  self.method == 'QPS-Diagonale2':
            sol, info = self.solvePlaneSearch_Newton(var=4) 
        elif  self.method == 'QPS-Diagonale3':
            sol, info = self.solvePlaneSearch_Newton(var=5)
        elif  self.method == 'QPS-Diagonale4':
            sol, info = self.solvePlaneSearch_Newton(var=6)
        elif  self.method == 'QPS-Diagonale5':
            sol, info = self.solvePlaneSearch_Newton(var=7)
        elif  self.method == 'QPS-Diagonale6':
            sol, info = self.solvePlaneSearch_Newton(var=8)
        elif  self.method == 'QPS-Diagonale7':
            sol, info = self.solvePlaneSearch_Newton(var=9)                                           
        elif  self.method == 'QPS-Diagonale8':
            sol, info = self.solvePlaneSearch_Newton(var=10)
        elif  self.method == 'QPS-Diagonale_Hd':
    ...
    def solvePlaneSearch_randdir(self, var=0):
        xk = self.problem.get_x0()
        n_iters = 0
        num_fails = 0
        xk_1 = np.copy(xk)
        ...
        while True:
            ...
            #if fExp >= f:
                fExp=f
                dk = np.random.normal(size=self.problem.n)
                dnr = np.linalg.norm(dk)
                while dnr <= 0.0:
                    dk = np.random.normal(size=self.problem.n)
                    dnr = np.linalg.norm(dk)
                dk = dk/dnr
                #print('size(dk)',len(dk))
                ab,fExp = self.bidimensional_search_Newton(fExp,f_1, g_1, xk, -g, dk ,n_iters, ab[0], ab[1], alpha_1, beta_1, var, deriv_free=True, maxfev=5)
                #input()
                #if (ab[0]< 1.e-12):
                #    ab[0]=1.e-12
                    #print('alfa piccolo')
    ...
    def solvePlaneSearch_Newton(self, var=0):
        ...
        while True:
            ...
            xk = new_x
            n_iters += 1
            #print('var=',var,"iters", n_iters, "f", f, "g_norm", g_norm)
        return xk, {"iters": n_iters, "f": f, "g_norm": g_norm, "nfails": num_fails, "nnegeig": self.nnegeig, "cosmax": '----'}
...
solvers = ['QPS-Diagonale_Hd','QPS-Diagonale1','QPS-Diagonale2','QPS-Diagonale3','QPS-Diagonale4','QPS-Diagonale5','QPS-Diagonale6','QPS-Diagonale7','QPS-Diagonale8','QPS-Newton','QPS-Newton_Matteo','scipy_lbfgs', 'scipy_cg']
#solvers = ['QPS-Diagonale']
#solvers = ['QPS-Newton_Matteo','QPS-Diagonale_Hd']
#solvers = ['QPS-Newton']
#solvers = ['QPS_New']
#solvers = ['QPS-Diagonale_Hd']
...
for p in problems:
    ...
    #print('2572: ',time()-tic)
    #for i in range(641):
    	#f = np.linalg.norm(P.x0,2)
    	#f =np.dot(P.x0.T,P.x0)
    	#a=1
    	#b=2
    	#f=a*b
    #print('641: ',time()-tic)
