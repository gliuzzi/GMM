...
class der_free_method:
	def __init__(self,funct,lb,ub,g_norm,maxiter=10000,maxfev=1000,tol=1.e-6,iprint=-1):
		self.funct = funct
		self.lb = lb
		self.ub = ub
		self.g_norm=g_norm
	...
	def linesearchbox_cont(self,n,x,f,d,alfa_d,j,alfa_max,iprint,bl,bu,nf):
		...
		if abs(alfa_d[j-1]) <= 1.e-24*min(1.0,alfa_max):
			alfa = 0.0
			if iprint >= 1:
				print('  alfa piccolo')
		...
		for ielle in range(1,3):
			...
			if abs(alfa) <= 1.e-24*min(1.0,alfa_max):
			...
			# expansion step
				...
				while True:
					...
					if iprint >= 1:
						print(' fzex=%e  alfaex=%e' % (fzdelta,alfaex))
					...
					if fzdelta < fpar:
						...
						if iprint>= 1:
							print(' accetta punto fz =%f   alfa =%e' % (fz,alfa))
	...
	#---- choice of the starting stepsizes along the directions --------
		...
		for i in range(0,n):
			alfa_d[i] = 1.e-0/min(max(1.e-16,g_norm),1.e16)  #max(1.e-12,min(1.0,abs(x[i])))

			if iprint >= 1:
				print(' alfainiz(%d)=%e' % (i,alfa_d[i]))
		alfa_max = max(alfa_d)
		alfa_stop=tol*alfa_max
		f   = self.funct(x)
		nf += 1
		i_corr = 1
		fstop[i_corr-1] = f
