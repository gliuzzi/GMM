...
class Solver:
    ...
    def bidimensional_search_Newton(self,finiz, f_1, g_1, x, d1, dnr1, d2,n_iter_glob , alpha0, beta0, alpha_1, beta_1, var,multistart=0, deriv_free=False, maxfev=10):
        def f2(ab):
            return self.f(x + ab[0] * d1/dnr1 + ab[1] * d2)

        def g2(ab):
            g = self.g(x + ab[0] * d1/dnr1 + ab[1] * d2)
            return [np.dot(g, d1/dnr1), np.dot(g, d2)]

        def fg2(ab):
            g = self.g(x + ab[0] * d1/dnr1 + ab[1] * d2)
            return self.f(x + ab[0] * d1/dnr1 + ab[1] * d2), np.array(np.dot(g, d1/dnr1), np.dot(g, d2))
        ...
        def inner_NN(ab,ab_1,f_1,g_1):
            ...
            #print('alpha0=',alpha0)
            # print('grad app',gapp)			
            NN=Newton(2,finiz,f_1,g_1,ab,ab_1,1.e-3*self.grad_tol*self.grad_tol,1,False,f2,d1,dnr1,d2,n_iter_glob,var)
            f,x=NN.minimize()
            return f,x
        #print('alpha0=',alpha0)
        #print('beta0=',beta0)
    ...
    def solvePlaneSearch_Newton(self, var=0):
        ...
        while True:
            ...
            #if fExp >= f:
                fExp=f
                ab,fExp = self.bidimensional_search_Newton(fExp,f_1, g_1, xk, -g, gnr,xk - xk_1,n_iters, ab[0], ab[1], alpha_1, beta_1, var, deriv_free=True, maxfev=5)
                #input()
                #if (ab[0]< 1.e-12):
                #    ab[0]=1.e-12
                    #print('alfa piccolo')
            ...
            new_x = xk - alpha * g/gnr + beta * (xk - xk_1)
            xk_1 = xk
            f_1, g_1 = f, g
            xk = new_x
            n_iters += 1
        ...
        while True:
            ...
            if True:
                ...
                while dnr <= 0.0:
                    ...
                    dnr = np.linalg.norm(dk)
                dk = dk/dnr
                ab,fExp = self.bidimensional_search_Newton(fExp,f_1, g_1, xk, -g, gnr,dk ,n_iters, ab[0], ab[1], alpha_1, beta_1, var, deriv_free=True, maxfev=5)
                #input()
                #if (ab[0]< 1.e-12):
                #    ab[0]=1.e-12
                    #print('alfa piccolo')
...
#solvers = ['QPS-Newton']
#solvers = ['QPS-randdir']
#solvers = ['QPS-randdir','QPS-Newton_Matteo']
#solvers = ['QPS_New']
#solvers = ['QPS_New']
