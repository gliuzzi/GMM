...
class Solver:
    ...
    def bidimensional_search_Newton(self,finiz, f_1, g_1, x, d1, d2,n_iter_glob , alpha0, beta0, alpha_1, beta_1, multistart=0, deriv_free=False, maxfev=10):
        def f2(ab):
            return self.f(x + ab[0] * d1 + ab[1] * d2)
        ...
        def inner_NN(ab,ab_1,f_1,g_1):
            ...
            # #print('d1=',d1)
            # print('grad app',gapp)			
            NN=Newton(2,finiz,f_1,g_1,ab,ab_1,1.e-3*self.grad_tol*self.grad_tol,1,False,f2,d1,d2,n_iter_glob)
            f,x=NN.minimize()
            return f,x
    ...
    def solvePlaneSearch_Newton(self):
        ...
        while True:
            ...
            # if np.dot(g.T,(xk-xk_1)) > 0.:
                ...
                ##print(' gnorm=',g_norm)
                #g = g / g_norm*1.e+9
            if g_norm < self.grad_tol or n_iters >= self.max_iters or f == -np.inf:
                break
            ...
            if True:
                ...
                fExp=f
                #print('f prima=',f)
                ab,fExp = self.bidimensional_search_Newton(fExp,f_1, g_1, xk, -g, xk - xk_1,n_iters, ab[0], ab[1], alpha_1, beta_1, deriv_free=True, maxfev=5)
                #print('f dopo=',fExp)
                #print('ab[0]=',ab[0])
                #print('ab[1]=',ab[1])
                #input()
